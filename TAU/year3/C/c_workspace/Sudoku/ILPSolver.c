...
int solve(int m, int n, int **gameBoard) {
    ...
    GRBenv*   env   = NULL;
    GRBmodel* model = NULL;
    int       constraint_indices[N]; // Variable indices for non-zero values in the new constraint
    double    val[N];
    double    lower_bounds[N * N * N];
    char      vartype[N * N * N];
    char*     varnames[N * N * N];
    char      namestorage[10 * N * N * N];
    char*     cursor;
    int       optimstatus;
    double    objval;
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j < N; j++) {
            for (v = 0; v < N; v++) {
                ...
                vartype[i * N * N + j * N + v] = GRB_BINARY;

                varnames[i * N * N + j * N + v] = cursor;
                sprintf(varnames[i * N * N + j * N + v], "x[%d,%d,%d]", i, j, v + 1);
                cursor += strlen(varnames[i * N * N + j * N + v]) + 1;
            }
        }
    }
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j < N; j++) {
            for (v = 0; v < N; v++) {
                constraint_indices[v] = i * N * N + j * N + v;
                val[v] = 1.0;
            }

            error = GRBaddconstr(model, N, constraint_indices, val, GRB_EQUAL, 1.0, NULL);
            if (error) {
    ...
    for (v = 0; v < N; v++) {
        for (j = 0; j < N; j++) {
            for (i = 0; i < N; i++) {
                constraint_indices[i] = i * N * N + j * N + v;
                val[i] = 1.0;
            }
    ...
    for (v = 0; v < N; v++) {
        for (i = 0; i < N; i++) {
            for (j = 0; j < N; j++) {
                constraint_indices[j] = i * N * N + j * N + v;
                val[j] = 1.0;
            }
    ...
    // m  number of rows in each box
    // n  number of columns in each box
            ...
            for (jg = 0; jg < n; jg++) {
                ...
                for (i = ig * m; i < (ig + 1) * m; i++) {
                    for (j = jg * n; j < (jg + 1) * n; j++) {
                        constraint_indices[count] = i * N * N + j * N + v;
                        val[count] = 1.0;
                        count++;
                    }
                }
    ...
    // N * N * N - number of variables, the size of "sol" should match
